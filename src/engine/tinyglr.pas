...
type
  ...
  TglrVec22Dynamics = TglrList<TglrVec22KeyFrame>;
  ...
  protected
    ...
    function GetParticle(): Integer;
    procedure ReleaseParticle(aIndex: Integer);
    procedure PrepareParticle(aIndex: Integer);

    function GetSize(aTime: Single): Integer;
    function GetVelocity(aTime: Single): Integer;
    function GetVelocityDispersion(aTime: Single): Integer;
    function GetEmitterShapeBox(aTime: Single): Integer;
    function GetEmitterShapeCircle(aTime: Single): Integer;
    function GetColor(aTime: Single): Integer;
    function GetParticleLifeTime(aTime: Single): Integer;
    function GetParticlesCount(aTime: Single): Integer;
    function GetParticlesPerSec(aTime: Single): Integer;
  public
    // Dynamics
    Size: TglrVec22Dynamics;
    ...
    ParticlePerSec: TglrIntegerDynamics;

    EmitterShapeType: TglrParticleEmitterShapeType2D;
    Enabled: Boolean;
    Time, Duration: Single;
    ParticleTexRegion: PglrTextureRegion;
...
begin
  fActiveParticles += 1;
  for i := 0 to fParticles.Count - 1 do
    if not fParticles[i].enabled then
      Exit(i);

  fParticles.Add(fParticles[0]);
  i := fParticles.Count - 1;
  Exit(i);
end;

procedure TglrParticleEmitter2D.ReleaseParticle(aIndex: Integer);
begin
  fParticles.BoundsCheck(aIndex);
  with fParticles[aIndex] do
    enabled := False;
  fActiveParticles -= 1;
end;

procedure TglrParticleEmitter2D.PrepareParticle(aIndex: Integer);
begin
  with fParticles[aIndex] do
  begin
    enabled := True;
    Log.Write(lCritical, 'PrepareParticle is not implemented');
  end;
end;

function TglrParticleEmitter2D.GetSize(aTime: Single): Integer;
var
  i: Integer;
begin
  ...
  for i := Size.Count - 1 downto 0 do
    if (aTime >= Size[i].t) then
      Exit(i);
  Log.Write(lCritical, 'ParticleEmitter2D: No keyframe at Size');
end;

function TglrParticleEmitter2D.GetVelocity(aTime: Single): Integer;
var
  i: Integer;
begin
  if Velocity.Count = 1 then
    Exit(0);

  for i := Velocity.Count - 1 downto 0 do
    if (aTime >= Velocity[i].t) then
      Exit(i);
  Log.Write(lCritical, 'ParticleEmitter2D: No keyframe at Velocity');
end;

function TglrParticleEmitter2D.GetVelocityDispersion(aTime: Single): Integer;
var
  i: Integer;
begin
  if VelocityDispersionAngle.Count = 1 then
    Exit(0);

  for i := VelocityDispersionAngle.Count - 1 downto 0 do
    if (aTime >= VelocityDispersionAngle[i].t) then
      Exit(i);
  Log.Write(lCritical, 'ParticleEmitter2D: No keyframe at VelocityDispersionAngle');
end;

function TglrParticleEmitter2D.GetEmitterShapeBox(aTime: Single): Integer;
var
  i: Integer;
begin
  if EmitterShapeType = peCircle then
    Log.Write(lCritical, 'ParticleEmitter2D: Have tried to get keyframe for shape `Box`, but emitter shape type is `Circle`');

  if EmitterShapeBox.Count = 1 then
    Exit(0);

  for i := EmitterShapeBox.Count - 1 downto 0 do
    if (aTime >= EmitterShapeBox[i].t) then
      Exit(i);
  Log.Write(lCritical, 'ParticleEmitter2D: No keyframe at EmitterShapeBox');
end;

function TglrParticleEmitter2D.GetEmitterShapeCircle(aTime: Single): Integer;
var
  i: Integer;
begin
  if EmitterShapeType = peBox then
    Log.Write(lCritical, 'ParticleEmitter2D: Have tried to get keyframe for shape `Circle`, but emitter shape type is `Box`');

  if EmitterShapeCircle.Count = 1 then
    Exit(0);

  for i := EmitterShapeCircle.Count - 1 downto 0 do
    if (aTime >= EmitterShapeCircle[i].t) then
      Exit(i);
  Log.Write(lCritical, 'ParticleEmitter2D: No keyframe at EmitterShapeCircle');
end;

function TglrParticleEmitter2D.GetColor(aTime: Single): Integer;
var
  i: Integer;
begin
  ...
  for i := Color.Count - 1 downto 0 do
    if (aTime >= Color[i].t) then
      Exit(i);
  Log.Write(lCritical, 'ParticleEmitter2D: No keyframe at Color');
end;

function TglrParticleEmitter2D.GetParticleLifeTime(aTime: Single): Integer;
var
  i: Integer;
begin
  if LifetimeMinMax.Count = 1 then
    Exit(0);

  for i := LifetimeMinMax.Count - 1 downto 0 do
    if (aTime >= LifetimeMinMax[i].t) then
      Exit(i);
  Log.Write(lCritical, 'ParticleEmitter2D: No keyframe at LifetimeMinMax');
end;

function TglrParticleEmitter2D.GetParticlesCount(aTime: Single): Integer;
var
  i: Integer;
begin
  if ParticlesCount.Count = 1 then
    Exit(0);

  for i := ParticlesCount.Count - 1 downto 0 do
    if (aTime >= ParticlesCount[i].t) then
      Exit(i);
  Log.Write(lCritical, 'ParticleEmitter2D: No keyframe at ParticleCount');
end;

function TglrParticleEmitter2D.GetParticlesPerSec(aTime: Single): Integer;
var
  i: Integer;
begin
  if ParticlePerSec.Count = 1 then
    Exit(0);

  for i := ParticlePerSec.Count - 1 downto 0 do
    if (aTime >= ParticlePerSec[i].t) then
      Exit(i);
  Log.Write(lCritical, 'ParticleEmitter2D: No keyframe at ParticlePerSec');
end;
...
begin
  ...
  ParticlePerSec := TglrIntegerDynamics.Create();
  ...
  fParticles := TglrParticles2D.Create(128);
  FillChar(p, SizeOf(TglrParticle2D), 0);
  p.color := dfVec4f(1, 1, 1, 1);
  for i := 0 to 127 do
    fParticles.Add(p);
end;
...
//          pos += dfVec3f();
    ...
    index := GetParticlesCount(Time);
    dif := ParticlesCount[index].value - fActiveParticles;
    if (dif > 0) then
    begin
      for i := 0 to dif - 1 do
        PrepareParticle(GetParticle());
    end;
  end;
end;
